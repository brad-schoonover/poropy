...
/**
 *  @file   ParamDB.cc
 *  @brief  ParamDB class member definitions
 */
//---------------------------------------------------------------------------//
...
//---------------------------------------------------------------------------//
ParamDB::ParamDB(std::string name)
  : d_name(name)
{
  /* ... */
}
...
void ParamDB::display() const
{
  ...
  }
  {
    ...
    std::map<string, SP_db>::const_iterator it = d_data_db.begin();
...
//---------------------------------------------------------------------------//
int ParamDB::size(int type) const
{
  int value = 0;
  if (type == INT)
